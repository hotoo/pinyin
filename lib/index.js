...
class NodePinyin extends Pinyin {
  ...
  convert (hans, options) {
    ...
    let phrases = options && options.segment ? segment(hans, options.segmentArgs) : hans;
    let pys = [];
    let nohans = "";
...
function segment(hans, args) {
  ...
  args = args || [];
  args = [hans].concat(args);
  return jieba.cut.apply(jieba, args);
}
