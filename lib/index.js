...
class NodePinyin extends Pinyin {
  ...
  convert(hans, options) {
    ...
    const opts = assign({}, Pinyin.DEFAULT_OPTIONS, options);
    const phrases = opts && opts.segment ? segment(hans) : hans;
    let pys = [];
    let nohans = "";

    for (let i = 0, firstCharCode, words, l = phrases.length; i < l; i += 1) {
      words = phrases[i];
